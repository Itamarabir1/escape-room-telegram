...
# Logger for env load and config validation
...
class Config:
    ...
    # Fallback origin when WEBAPP_URL is not set (CORS). Production frontend URL.
    ...
    # --- AWS (optional, only if used) ---
    AWS_ACCESS_KEY_ID: str = os.getenv("AWS_ACCESS_KEY_ID", "")
    AWS_SECRET_ACCESS_KEY: str = os.getenv("AWS_SECRET_ACCESS_KEY", "")
    AWS_REGION: str = os.getenv("AWS_REGION", "eu-north-1")

    # Redis: Docker Compose uses REDIS_URL=redis://redis:6379/0; Render uses REDIS_INTERNAL_URL or addon REDIS_URL.
    REDIS_URL: str = (
        os.getenv("REDIS_URL")
...
# Minimal validation after config load
